 ...
 */
@Ignore
public class CreditRegularDaoImplTest {
	private static JdbcPersistentStore jps;
	private CreditRegular testInstrumentSave = new CreditRegular();
	...
	public void saveIntoDatabase() {
		...
		Frequency f = Frequency.FOUR_MONTHS;
		testInstrumentSave.setFrequency(f);
		testInstrumentSave.setMaturity(new GregorianCalendar(2020, 10, 10));
		jps.startTransaction();
		jps.getInstrumentDao().save(testInstrumentSave);
		jps.commitTransaction();
	}

	public CreditRegular loadFromDatabase() {
		CreditRegular testInstrumentLoad = new CreditRegular();

		jps.startTransaction();
		testInstrumentLoad = (CreditRegular) jps.getInstrumentDao().loadById(id, new GregorianCalendar(2016, 5, 5));
		jps.commitTransaction();
	...
	public void testSaveLoadInstrument() {
		...
		saveIntoDatabase();
		CreditRegular testInstrument = loadFromDatabase();
		...
		assertEquals("Maturity is not the same", testInstrumentSave.getMaturity(), testInstrument.getMaturity());
	...
	public void testHistorization() {
		testInstrumentSave = new CreditRegular();
		testInstrumentSave.setID(id);
		...
		CreditRegular testInstrument = new CreditRegular();

		jps.startTransaction();
		testInstrument = (CreditRegular) jps.getInstrumentDao().loadById(id, time);
		jps.commitTransaction();
